...
\begin{homeworkProblem}[: Procedimientos para crear y mostrar un avión (2 puntos)]
  Crea un procedimiento de nombre \texttt{CREAR\_AVION\_TIPO(avion\_id OUT number)} que dé de alta un avión y devuelva el \texttt{ID} de dicho avión recién creado. Los \textit{serial number}, así como los identificadores de cada pieza, se extraerán de la secuencia correspondiente, y por eso no se muestran en los datos a insertar.
